...
import Button from "../components/shared/Atomics/Button";
				...
				<div className="hero place-content-start rounded-lg bg-gradient-to-r from-neutral to-transparent">
					<div className="hero-content flex-row sm:flex-col md:flex-col xl:gap-6 xxl:gap-10">
						...
						<div className="sm:self-start">
							<p className="first-letter:uppercase">{data?.public ? "public playlist" : "private playlist"}</p>
							<h1 className="text-5xl font-bold">{data?.title}</h1>
							<p className="my-2 text-lg">{Array.isArray(data.tracks) ? data?.tracks?.length : 0} tracks</p>
							<p className="my-4">
								Created by <Link className="font-bold text-base-content">{data?.creator?.username}</Link>
							</p>
						</div>
					</div>
				</div>
